...
public class DAO {
	...
	public ArrayList<VO> getMemberList() {
		...
		try {
			conn = ConnUtil.getConnection();
			sql = "SELECT NUM, USERNAME, TITLE, TIME, HIT, INDENT from notice order by ref desc, step asc";
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
	...
	public VO getView(int idx) {
		...
		try {
			conn = ConnUtil.getConnection();
			sql = "SELECT USERNAME, TITLE, MEMO, TIME, HIT, PASSWORD, REF, INDENT, STEP FROM notice WHERE NUM=?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, idx);
			rs = pstmt.executeQuery();
	...
	public boolean checkPassword(VO vo, int idx) {
		...
		try {
			conn = ConnUtil.getConnection();
			sql = "SELECT NUM FROM notice where NUM=? and PASSWORD=?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, idx);
	...
	public void UpdateStep(int ref, int step) {
		...
		try {conn = ConnUtil.getConnection();
			sql = "UPDATE notice SET STEP=STEP+1 where REF=? and STEP>?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, ref);
			pstmt.setInt(1, step);
			pstmt.executeUpdate();
